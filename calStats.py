...
def cal_chi_result(dataSet, tcol, trow, general=False, beauty=True):
    '''
...
#             chi2 = np.round(stats.chi2_contingency(tableDict.iloc[:-1,:-1])[1],3) #[1]:chi2,[2]:df
            ...
            if beauty:
                ###====下面内容为优化表格====####
                #               #将百分数添加到字段后面
                tableDict.iloc[:, :-2] = tableDict.iloc[:, :-
                                                        2].astype(str)+'('+percent.iloc[:, :].round(2).astype(str)+')'
                # 在为小表格添加一个新行
#                 pd.concat([pd.DataFrame(['随便写点儿什么'],index=[tda],columns=['rp']), tableDict]).drop('rp',axis=1)
            ...
            if general:
                genDF = pd.DataFrame([beautyP], columns=[r+'-p'], index=[tda])
